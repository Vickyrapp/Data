...
if (pageId == "JFTKosakata"){
	...
	let correctPassword = '';
	fetch('https://raw.githubusercontent.com/Vickyrapp/Data/main/password.js')
	.then(response => {
		if (!response.ok) {
	...
	.then(data => {
		correctPassword = data.trim(); // Trim untuk menghapus spasi
		checkStoredPassword();
	})
	.catch(error => {
	...
	function checkPassword() {
		...
		if (inputPassword === correctPassword && inputPassword !== "") {
		...
		questions.forEach((q, i) => {
			...
			if (selected) {
				if (selected.value === correctAnswer) {
					...
					x += `<div class="item"><b>${bab.content[i].y}</b><b>5x</b></div>`;
				}
			} else {
				feedbackDiv.textContent = 'Belum dijawab!';
				feedbackDiv.style.color = 'orange';
				x += `<div class="item"><b>${bab.content[i].y}</b><b>10x</b></div>`;
			}
		});
		if (x !== undefined) {
			failed.innerHTML = x;
	...
	const credits = [
		{ text: "Vicky", audio: "media/voice1.mp3", translateX: "40%", translateY: "50%" },
		{ text: "Faros", audio: "media/voice2.mp3", translateX: "-20%", translateY: "40%" },
		{ text: "Bawon", audio: "media/voice3.mp3", translateX: "-40%", translateY: "40%" },
		{ text: "Dehan", audio: "media/voice4.mp3", translateX: "40%", translateY: "10%" },
		{ text: "Imam", audio: "media/voice5.mp3", translateX: "40%", translateY: "-60%" },
		{ text: "Yoga", audio: "media/voice6.mp3", translateX: "-20%", translateY: "-40%" },
		{ text: "Fauzi", audio: "media/voice7.mp3", translateX: "-50%", translateY: "-30%" }
	];
	...
	const appInfo = [
		{
			version: "1.0",
			description: ["Pembuatan Aplikasi.", "Menambahkan kartu kanji"]
		},
		{
			version: "1.1",
			description: ["Menambahkan fitur kuis", "Mengoptimalkan Aplikasi"]
		},
		{
			version: "1.2",
		...
		{
			version: "1.3",
			description: ["Menambahkan main menu", "Menambahkan fitur kosakata", "Mengoptimalkan tampilan"]
		},
		{
			version: "1.4",
	...
	function populateAbout() {
		...
		appInfo.forEach(info => {
			...
			versionElem.textContent = `Versi: ${info.version}`;
